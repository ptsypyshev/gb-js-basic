...
function handleClick(clickedButtonEvent) {
    ...
    // const cardNode = clickedButtonEvent.target.classList.contains(".product");
    ...
    const card = {
        ...
        productName: cardNode.querySelector(".productName"), // здесь , который внутри .product
 ...
 * @param {{button: Element, img: Element, wrap: HTMLElement, productName: Element}} card
...
function hideMoreText(card) {
    ...
    let cardDescription = card.wrap.querySelector(".desc");
...
function showMoreText(card) {
    ...
    let imgHeight = card.img.clientHeight;
    card.img.style.display = "none";
    // 6.1 сохраняем произвольный текст в переменную
    let cardDescription = "It is a product description!";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    let divDescription = `<div class="desc">${cardDescription}</div>`;
    card.productName.insertAdjacentHTML("afterend", divDescription)
    card.productName.nextElementSibling.style.height = `${imgHeight}px`;
